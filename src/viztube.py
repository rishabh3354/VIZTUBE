...
from PyQt5.QtCore import QUrl, QSettings, QStringListModel, QProcess, QCoreApplication
...
PRODUCT_NAME = "VIZTUBE"
THEME_PATH = '/snap/viztube/current/'
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("VizTube")
        self.settings = QSettings("warlordsoft", "viztube")
        self.tip_count = -1
        ...
        self.youtube_setting_ui.ui.download_path_edit_2.setText(self.Default_loc + "/VIZTUBE")
        self.youtube_setting_ui.ui.download_path_edit_playlist.setText(self.Default_loc_playlist + "/VIZTUBE")
        ...
        self.ui.menuHome.triggered.connect(self.get_home_page)
        self.ui.menu_Paste_URL.triggered.connect(self.open_url_dialog)
        self.ui.menu_Download_History.triggered.connect(self.show_downloads_page)
        self.ui.menu_Your_Plan.triggered.connect(self.account_page)
        self.ui.menu_About_App.triggered.connect(self.about_page)
        self.ui.menu_Youtube_Settings.triggered.connect(self.open_yt_setting_page)
        self.ui.menu_System_Monitors.triggered.connect(self.show_net_speed)
        self.ui.menu_Your_Downloads.triggered.connect(self.show_downloads_page)
        self.ui.menu_exit.triggered.connect(self.quit_app)

        self.ui.menu_Tip.triggered.connect(self.suggestion_info_popup)
        self.ui.dock_back_home.clicked.connect(self.back_page)

    def open_menu(self):
        if self.back_track_index != self.ui.stackedWidget.currentIndex():
            self.back_track_index = self.ui.stackedWidget.currentIndex()
        self.ui.stackedWidget.setCurrentIndex(7)
        self.ui.dock_back_home.setEnabled(True)

    def quit_app(self):
        QCoreApplication.exit(0)

    def back_page(self):
        self.ui.stackedWidget.setCurrentIndex(self.back_track_index)
    ...
    def server_info_popup(self):
        title = "VIZTUBE SERVER INFO!"
        message = "Change the server name if you are facing any issues related to loading Home page/Search on youtube.\n\n" \
                  "Reason: It might be possible that VIZTUBE server is down for temporary basis on your country.\n\n" \
    ...
    def suggestion_info_popup(self, message=None):
        ...
        self.msg.setStyleSheet("background-color:#263a4e;color:#eaeaea;")
        self.msg.setIcon(QMessageBox.Information)
        self.msg.setText("VIZTUBE Tips and Tricks!")
        if message:
        ...
        message_list = [
            ...
            "Change VIZTUBE server from the App settings if you are facing server down issues in your country.",
            ]
        try:
    ...
    def server_error_handle(self, msg):
        self.ui.home_progress_bar.setRange(0, 1)
        self.popup_message(title="Could not connect to the server!", message="VIZTUBE server is currently busy,"
    ...
    def load_settings(self):
        ...
        if self.settings.contains("default_server"):
            ...
            self.youtube_setting_ui.ui.server.setCurrentText(SERVER_REVERSE.get(self.default_server, "VIZTUBE-SERVER-1"))
    ...
    def open_download_path(self):
        ...
        if folder_loc:
            if check_default_location(folder_loc):
                self.youtube_setting_ui.ui.download_path_edit_2.setText(folder_loc + "/VIZTUBE")
    ...
    def open_download_path_playlist(self):
        ...
        if folder_loc:
            if check_default_location(folder_loc):
                self.youtube_setting_ui.ui.download_path_edit_playlist.setText(folder_loc + "/VIZTUBE")
    ...
    def clear_download_history_all(self):
        try:
            video_history_path = self.Default_loc + "/VIZTUBE/.downloads/download_data.json"
            os.remove(video_history_path)
        except Exception as e:
            pass
        try:
            playlist_history_path = self.Default_loc_playlist + "/VIZTUBE/.downloads/download_data.json"
            os.remove(playlist_history_path)
        except Exception as e:
            pass
        if self.delete_source_file:
            try:
                video_file_path = self.Default_loc + "/VIZTUBE"
                shutil.rmtree(video_file_path)
            except Exception as e:
                pass
            try:
                playlist_video_path = self.Default_loc_playlist + "/VIZTUBE"
    ...
    def delete_entry_from_list(self, delete_source_file=False):
        ...
        if c_index != -1:
            ...
            delete_location = str(poped_item.get("download_path")).split("VIZTUBE")[0]
            video_info_copy_1 = []
    ...
    def redirect_to_rate_snapstore(self):
        QDesktopServices.openUrl(QUrl("snap://viztube"))
    ...
    def check_your_plan(self):
        if not self.is_plan_active:
            ...
            self.msg.setInformativeText(
                "In VIZTUBE free version, HD+ video quality option is not available. But you can still download SD quality videos.\n"
