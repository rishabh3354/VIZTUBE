...
def get_download_path(location,
    ...
    try:
        if location:
            if thumbnail:
                location += '/VIZTUBE/.thumbnails'
                os.makedirs(location, exist_ok=True)
            elif process_dash_stream:
                location += '/VIZTUBE/.dash_stream'
                os.makedirs(location, exist_ok=True)
            elif download_video_path:
                location += '/VIZTUBE/videos'
                os.makedirs(location, exist_ok=True)
            elif download_playlist_video_path:
                location += '/VIZTUBE/playlist/videos'
                os.makedirs(location, exist_ok=True)
            elif download_playlist_audio_path:
                location += '/VIZTUBE/playlist/audio'
                os.makedirs(location, exist_ok=True)
            elif download_audio_path:
                location += '/VIZTUBE/audio'
                os.makedirs(location, exist_ok=True)
            else:
                location += '/Downloads'
            return location
            ...
            if HOME != '':
                if thumbnail:
                    HOME += '/Downloads/VIZTUBE/.thumbnails'
                    os.makedirs(HOME, exist_ok=True)
                elif process_dash_stream:
                    HOME += '/Downloads/VIZTUBE/.dash_stream'
                    os.makedirs(HOME, exist_ok=True)
                elif download_video_path:
                    HOME += '/Downloads/VIZTUBE/videos'
                    os.makedirs(HOME, exist_ok=True)
                elif download_audio_path:
                    HOME += '/Downloads/VIZTUBE/audio'
                    os.makedirs(HOME, exist_ok=True)
                else:
                    HOME += '/Downloads'
            else:
        ...
        HOME = QProcessEnvironment().systemEnvironment().value('SNAP_REAL_HOME') + "/Downloads/VIZTUBE/"
